...
export default function TeknikHizmetlerPage() {
  ...
  async function loadData() {
    ...
    // Servisleri ve müşterileri yükle
    const [servicesRes, customersRes] = await Promise.all([
      supabase.from('technical_services').select('*').order('renewal_date', { ascending: true }),
  ...
  const stats = {
    ...
    brands: new Set(services.map(s => s.customer_id).filter(Boolean)).size,
    upcoming: services.filter(s => {
      const days = getDaysDiff(s.renewal_date)
      return days > 0 && days <= 30
    }).length,
    overdue: services.filter(s => getDaysDiff(s.renewal_date) < 0).length
  }
      ...
      <div className="glass-card rounded-xl p-4 border border-zinc-200 dark:border-white/10 mb-6">
        <div className="flex items-center gap-4">
          <div className="relative flex-1">
            ...
            <Input 
              type="text" 
              placeholder="Hizmet veya marka ara..." 
              value={searchQuery}
                      ...
                      <div className="flex items-center gap-2">
                        {service.customer && (
                          <>
                            ...
                            <span className="text-xs text-zinc-500">{service.customer.name}</span>
                          </>
                        )}
                        {service.price && (
                          <span className="text-xs text-zinc-400 font-mono">
                            ₺{service.price.toLocaleString('tr-TR')}/yıl
                          </span>
                        )}
                      </div>
                    </div>
